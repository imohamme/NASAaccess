      ...
      if(type == 'pr')
      {
        for(kk in 1:length(time_period))
        {
          ...
          if(format(timestart,"%m-%d") != "02-29")
          {
            ...
            cell.values[is.na(cell.values)] <- '-99.000' #filling missing data
          }

          else
          {
            ###the date is Feb 29th and NEX-GDPP has no value for it
            cell.values <- rep('-99.000',dim(FinalTable)[1])
      ...
      else
      {
        for(jj in 1:length(time_period))
        {
          ...
          if(format(timestart,"%m-%d") != "02-29")
          {
            ...
            cell.values_min[is.na(cell.values_min)] <- '-99.000' #filling missing data
            cell.values_max[is.na(cell.values_max)] <- '-99.000' #filling missing data
          }
          else
          {
            ###the date is Feb 29th and NEX-GDPP has no value for it
            cell.values_min <- rep('-99.000', dim(FinalTable)[1]) #filling missing data
            cell.values_max <- rep('-99.000', dim(FinalTable)[1]) #filling missing data

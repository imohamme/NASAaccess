...
#checking the netrc file existence and the logging information for NEX-GDPP first!
    ...
    if(length(grep("ftp.nccs.nasa.gov", readLines('~/_netrc')))==0)
      ...
      if (as.Date(start) >= as.Date('1950-01-01') &  as.Date(end) <= as.Date('2100-12-31') & slice == 'rcp85' | slice == 'rcp45' | slice == 'historical')
      {
        ...
        if(httr::status_code(GET(myurl))==200)
        {
          ...
          # downloading one file
          if(dir.exists('./temp/')==FALSE){dir.create('./temp/')}
          utils::download.file(quiet = T, method = 'curl', url = 'ftp.nccs.nasa.gov/BCSD/rcp85/day/atmos/tasmax/r1i1p1/v1.0/tasmax_day_BCSD_rcp85_r1i1p1_ACCESS1-0_2014.nc', destfile = paste('./temp/','tasmax_day_BCSD_rcp85_r1i1p1_ACCESS1-0_2014.nc',sep= ''), mode = 'wb', extra = '-n -c ~/.urs_cookies -b ~/.urs_cookies -L')
          #reading ncdf file
          ...
          #getting the julian day that corresponds to the dummy date
          Dummy_DATE_Julian <- as.numeric(format(DUMMY_DATE, "%j"))
          data<-ncdf4::ncvar_get(nc,'tasmax', start = c(1,1,Dummy_DATE_Julian) , count = c(-1, -1 ,1))
          #transpose the data
          data <- raster::t(data)
          #reorder the rows
          data<-data[ nrow(data):1, ]
        ...
        if(type == 'pr')
        {
          for(kk in 1:length(time_period))
          {
            ...
            julian_day <- as.numeric(format(time_period[kk], "%j"))
            filename <- paste(type,'_day_BCSD_',slice,'_r1i1p1_',model,'_',year,'.nc',sep = '')
            myurl <- paste(ftp,filename,sep = '')
            # downloading file
            if(dir.exists('./temp/')==FALSE){dir.create('./temp/')}
            if(file.exists(paste('./temp/',filename,sep= ''))==FALSE){utils::download.file(quiet = T, method = 'curl', url = myurl, destfile = paste('./temp/',filename,sep= ''), mode = 'wb', extra = '-n -c ~/.urs_cookies -b ~/.urs_cookies -L')}
            # Reading the ncdf file
            nc <- ncdf4::nc_open( paste('./temp/',filename,sep = '') )
            data <- ncdf4::ncvar_get(nc,type, start = c(1,1,julian_day) , count = c(-1, -1 ,1))
            #transpose the data
            data <- raster::t(data)
            #reorder the rows
            data<-data[ nrow(data):1, ]
        ...
        else
        {
          for(jj in 1:length(time_period))
          {
            ...
            data_max <- ncdf4::ncvar_get(nc_max,typemax, start = c(1,1,julian_day) , count = c(-1, -1 ,1))
            #transpose the data

...
#' @references Adler, R. F., G. J. Huffman, A. Chang, R. Ferraro, P.-P. Xie, J. Janowiak, B. Rudolf, U. Schneider, S. Curtis, D. Bolvin, A. Gruber, J. Susskind, P. Arkin, and
#'  E. Nelkin (2003), The Version-2 Global Precipitation Climatology Project (GPCP) Monthly Precipitation Analysis (1979–Present), J. Hydrometeorol., 4, 1147-1167, doi:10.1175/1525-7541(2003)004<1147:tvgpcp>2.0.co;2.
...
#' @rawNamespace import(terra, except = c('tail','head'))
#' @export
...
GLDASpolyCentroid=function(Dir='./SWAT_INPUT/', watershed ='LowerMekong.shp', DEM = 'LowerMekong_dem.tif', start = '2015-12-1', end = '2015-12-3')
{
  ...
  if (as.Date(start) >= as.Date('2000-01-01'))
  {
    ...
    for(jj in 1:dim(polys)[1])
    {
      ...
      filenameSWAT[[jj]]<-paste('temp',as.character(values(polys[jj,1])),sep='')
    ...
    for(kk in 1:length(time_period))
    {
      ...
      for(ll in 1:length(filenames))# Iterating over each subdaily data files
      {
      ...
      ## save time by cropping the world raster to the study DEM
          cropGLDAS<-terra::crop(x=GLDAS,y=watershed.elevation)
      ...
      ###daily records for each point
          dailytemp<-suppressWarnings(terra::extract(cropGLDAS, polys, weights=TRUE, fun=mean))

      ###obtain minimum daily data over the 3 hrs records
          mindailytemp<-apply(dailytemp[,-1],1,min)
      ...
      ###same for maximum daily
          maxdailytemp<-apply(dailytemp[,-1],1,max)

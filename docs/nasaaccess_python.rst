...
On a local machine the user should have installed the following programs as well as setting up a user account with `Earthdata <https://earthdata.nasa.gov/>`_.  The list below gives a summary of what is needed to be done prior to work with *NASAaccess* software on any local machine:
